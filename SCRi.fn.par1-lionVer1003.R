...
##########################################################################
##### SCRi.fn.par1-lionVer1001.R is the master function that performs the spatial capture-recapture analysis #######

## ***This version (Ver1003) improves upon the version (Ver1002) by activating dumprate  
## ***This version (Ver1002) improves upon the version (Ver1001) by preventing the unnecessary MCMC updates of beta.den parameter.  

# contin = Is the engine used for code continuation or for running a fresh analysis. If it is a continuation, then contin = 1, otherwise the default is 0. 
...
################### ADDED 8 May 2019 ####################
# "effort1" = Xeff1 = ntraps x ndays matrix or NULL. Assume log(eff). This is to estimate another source of effort. 
# "effort2" = Xeff2 = ntraps x ndays matrix or NULL. Assume log(eff). This is to estimate a third source of effort.
######################################################### 
...
SCRi.fn.par1 <-function(scrobj, contin=0, modelno=1, nc=3,ni=1100,burn=100,skip=2,nz=200,theta=NA,Msigma=1,Mb=0,Msex=0,Msexsigma = 0,Xeff=NULL, Xeff1=NULL, Xeff2=NULL, Xeff3=NULL, Xsex=NULL, ss.prob=NULL,coord.scale=1000,area.per.pixel=1,thinstatespace=1,maxNN=20,dumprate=1000){
############################################################
...
########################### ADDED 8 MAY 2019 ###########################
if(!is.null(Xeff1)){
Xeff1new<-array(0,dim=c(nind+nz,T,ntraps))
for(j in 1:M){
Xeff1new[j,1:T,1:ntraps]<-t(Xeff1)
}
Xeff1.tf<-TRUE
}
if(is.null(Xeff1)){
...
if(!is.null(Xeff2)){
Xeff2new<-array(0,dim=c(nind+nz,T,ntraps))
for(j in 1:M){
Xeff2new[j,1:T,1:ntraps]<-t(Xeff2)
}
Xeff2.tf<-TRUE
}
if(is.null(Xeff2)){
...
########################### ADDED 28 JUNE 2019 ###########################
if(!is.null(Xeff3)){
  Xeff3new<-array(0,dim=c(nind+nz,T,ntraps))
  for(j in 1:M){
    Xeff3new[j,1:T,1:ntraps]<-t(Xeff3)
  }
  Xeff3.tf<-TRUE
}
if(is.null(Xeff3)){
...
######################## ADDED 8 May 2019 ########################
Xeff1<-as.vector(Xeff1)
Xeff1<-Xeff1[msk2==1 & alive.trues==1]
Xeff2<-as.vector(Xeff2)
Xeff2<-Xeff2[msk2==1 & alive.trues==1]

######################## ADDED 28 June 2019 ########################
Xeff3<-as.vector(Xeff3)
Xeff3<-Xeff3[msk2==1 & alive.trues==1]
...
# Call the parallelizing package #
require(doMC) 
# Register the cores #
registerDoMC(nc)
	}
...
if(Msexsigma==0)
bsigma<- inits[[ch]][[1]]$bsigma
if(Msexsigma==1){
## bsigmainit <- rbeta(1,1,50)
bsigma <-c(inits[[ch]][[1]]$bsigma,inits[[ch]][[1]]$bsigma2)	
}

update.theta<-FALSE
if(is.na(theta)){
     theta<- inits[[ch]][[1]]$theta
     update.theta<-TRUE
 }

lam0<-inits[[ch]][[1]]$lam0
loglam0<-log(lam0)
beta.behave<-inits[[ch]][[1]]$beta.behave
beta.sex<-inits[[ch]][[1]]$beta.sex
# start beta1=0 so if there's no covariate this parameter is zeroed out
beta1<-inits[[ch]][[1]]$beta1.effort

########################### ADDED 8 MAY 2019 #############################
beta2<-inits[[ch]][[1]]$beta2.effort
beta3<-inits[[ch]][[1]]$beta3.effort
########################### ADDED 8 MAY 2019 #############################
beta4<-inits[[ch]][[1]]$beta4.effort
##########################################################################

Xsex <- unlist(inits[[ch]][[7]])
psi<-inits[[ch]][[1]]$psi ### but this would not matter because it gets computed
psi.sex <- inits[[ch]][[1]]$psi.sex
...
beta.den<-inits[[ch]][[1]]$beta.density
centers <- unlist(inits[[ch]][[5]]) 
...
print("Error trap 1.71")
################### MODIFIED 8 MAY 2019 ####################
out<-matrix(NA,nrow=(ni-burn)/skip,ncol=18)
dimnames(out)<-list(NULL,c("bsigma","sigma","bsigma2","sigma2","lam0","beta.behave","beta1(effort)","beta2(effort)","beta3(effort)","beta4(effort)","beta.sex","psi","psi.sex","Nsuper","theta","beta.density","D","D.adj"))
print("Error trap 1.72")
############################################################
...
print("Error trap 1.73")
m<-1

print("Error trap 1.7301")
	
LM1<-LM2<-matrix(0,nrow=M,ncol=length(indid.LM)/M)
	
print("Error trap 1.7302")
	
ones<-rep(1,ncol(LM1))
print("Error trap 1.731")
if(Msexsigma==0)
lp.sigma<-Msigma*bsigma*(c1+c2)^theta
if(Msexsigma==1)
lp.sigma<-bsigma[Xsex[indid]+1]*(c1+c2)^theta

print("Error trap 1.74")
acc.count<-0
delta<-.05

start.time <- Sys.time()

print("Error trap 1.8")
...
### update loglam0
################################# MODIFIED 8 MAY 2019 ########################
print("Error trap 1.9")
repeat{
  lp<-   loglam0 + Mb*beta.behave*prevcap - lp.sigma + beta1*Xeff + beta2*Xeff1 + beta3*Xeff2 + beta4*Xeff3 + Msex*beta.sex*Xsex[indid]
  loglam0c<-rnorm(1,loglam0,.1)
  lpc<-  loglam0c + Mb*beta.behave*prevcap - lp.sigma + beta1*Xeff + beta2*Xeff1 + beta3*Xeff2 + beta4*Xeff3 + Msex*beta.sex*Xsex[indid]
...
## theta must be less than 0.5 
  ...
  while(thetac<0.5 | thetac>1) {thetac<-rnorm(1,theta,.02)} 	
    ...
    lpc<-  loglam0 + Mb*beta.behave*prevcap - lp.sigmac + beta1*Xeff + beta2*Xeff1 + beta3*Xeff2  + beta4*Xeff3 + Msex*beta.sex*Xsex[indid]
    llvector<-lik.fn(lp,y1)
    llvector.new<-lik.fn(lpc,y1)
    if(length(llvector.new[!is.finite(llvector.new)])<1) break
}
...
lpc<-  loglam0 + Mb*beta.behave*prevcap - lp.sigma + beta1*Xeff + beta2*Xeff1 + beta3*Xeff2  + beta4*Xeff3 + Msex*beta.sexc*Xsex[indid]
llvector.new<- lik.fn(lpc,y1)
if(length(llvector.new[!is.finite(llvector.new)])<1) break
}
##########################################################################################
...
lpc<-  loglam0 + Mb*beta.behave.c*prevcap - lp.sigma + beta1*Xeff + beta2*Xeff1 + beta3*Xeff2 + beta4*Xeff3 + Msex*beta.sex*Xsex[indid]
llvector.new<- lik.fn(lpc,y1)
if(length(llvector.new[!is.finite(llvector.new)])<1) break
}
...
lpc<-  loglam0 + Mb*beta.behave*prevcap - lp.sigma + beta1c*Xeff + beta2*Xeff1 + beta3*Xeff2 + beta4*Xeff3 + Msex*beta.sex*Xsex[indid]
llvector.new<- lik.fn(lpc,y1)
if(length(llvector.new[!is.finite(llvector.new)])<1) break
}
...
### This block of code below deals with effort.
  repeat{
beta2c<- rnorm(1,beta2,.1)
lpc<-  loglam0 + Mb*beta.behave*prevcap - lp.sigma + beta1*Xeff + beta2c*Xeff1 + beta3*Xeff2 + beta4*Xeff3 + Msex*beta.sex*Xsex[indid]
llvector.new<- lik.fn(lpc,y1)
if(length(llvector.new[!is.finite(llvector.new)])<1) break
}
...
### This block of code below deals with effort.
  repeat{
beta3c<- rnorm(1,beta3,.1)
lpc<-  loglam0 + Mb*beta.behave*prevcap - lp.sigma + beta1*Xeff + beta2*Xeff1 + beta3c*Xeff2 + beta4*Xeff3  + Msex*beta.sex*Xsex[indid]
llvector.new<- lik.fn(lpc,y1)
if(length(llvector.new[!is.finite(llvector.new)])<1) break
}
...
if(Xeff3.tf){
  ### This block of code below deals with effort.
  repeat{
    beta4c<- rnorm(1,beta4,.1)
    lpc<-  loglam0 + Mb*beta.behave*prevcap - lp.sigma + beta1*Xeff + beta2*Xeff1 + beta3*Xeff2 + beta4c*Xeff3 + Msex*beta.sex*Xsex[indid]
    llvector.new<- lik.fn(lpc,y1)
    if(length(llvector.new[!is.finite(llvector.new)])<1) break
  }
...
################################## MODIFIED 8 MAY 2019 ###############################
lpc<-  loglam0 + Mb*beta.behave*prevcap - lp.sigmac + beta1*Xeff + beta2*Xeff1 + beta3*Xeff2 + beta4*Xeff3 + Msex*beta.sex*tmp.sex[indid]     ## + sex contribution if sex is in the model!
######################################################################################
...
###################################### MODIFIED 8 MAY 2019 ##################################
logmu<- loglam0 + Mb*beta.behave*prevcap - lp.sigma + beta1*Xeff + beta2*Xeff1 + beta3*Xeff2 + beta4*Xeff3 + Msex*beta.sex*Xsex[indid]
#############################################################################################
...
lam0, beta.behave, beta1, beta2, beta3, beta4, beta.sex,psi,psi.sex,sum(z),theta,beta.den,density,densityADJ)

####### MODIFIED 14 MAY 2020 TO ACTIVATE DUMPRATE AND WRITE COMPLETE FILES  ###############
if(i %% dumprate == 0){ # set dumprate in main model script to write complete files at specified interval
  ...
  out1<- list(mcmchist=out,G=G,Gunscaled=Gunscaled,traplocs=traplocs,Sout=Sout,zout=zout,Sexout=Sexout, likelihood=LLout,statespace=statespace,gof.data=gof.data,gof.new=gof.new,call=call,parms2report=parms.2.report) 
  out1<- lapply(out1, na.omit)
  ### Extract output into files ###
  ts <- format(Sys.time(), "%y%m%d_%H%M%S")
  ## Create a folder ##
  folderName <- paste("SHOresultsModel_",modelno,"_", ts,"CH",ch,"iter",i,sep="")
  dir.create(path=folderName)
  ## Store MCMC history ##
  fname <- paste(folderName,"/SHOmcmchist_",ts,"CH",ch,".csv",sep="")
  write.csv(file=fname,out1$mcmchist) 
  ## Store activity centres ##
  fname <- paste(folderName,"/AcCentres_",ts,"CH",ch,".csv",sep="")
  write.csv(file=fname,out1$Sout)
  ## Store real individuals ##
  fname <- paste(folderName,"/RealIndividuals_",ts,"CH",ch,".csv",sep="")
  write.csv(file=fname,out1$zout)
  ## Store sex of all individuals ##
  fname <- paste(folderName,"/SexIndividuals_",ts,"CH",ch,".csv",sep="")
  write.csv(file=fname,out1$Sexout)
  ## Store gof data ##
  fname <- paste(folderName,"/gofdata_",ts,"CH",ch,".csv",sep="")
  write.csv(file=fname,out1$gof.data)
  ## Store gof new ##
  fname <- paste(folderName,"/gofnew_",ts,"CH",ch,".csv",sep="")
  write.csv(file=fname,out1$gof.new)
  ## Store unscaled statespace for plotting ##
  fname <- paste(folderName,"/SSunscaled_",ts,"CH",ch,".csv",sep="")
  write.csv(file=fname,out1$Gunscaled)
  ...
  write.csv(file=fname,out1$likelihood)
  
  ########### END OF 14/05/2020 MODIFICATION TO ACTIVATE DUMPRATE
...
print("Error trap 3-post iterations")
...
print("Error trap 3.2")

if(contin==1){

folderName <- eval(parse(text=paste("dir",ch,sep="")))
setwd(folderName)

print("Error trap 3.3")	
### Append MCMC history ###
fname <- dir(pattern='^SHOmcmchist')[1]
write.table(out$mcmchist,file=fname,append=TRUE,quote=FALSE,sep=",", row.names=TRUE, col.names=FALSE)
print("Error trap 3.4")
### Append activity centres ###
fname <- dir(pattern='^AcCentres')[1]
write.table(out$Sout,file=fname,append=TRUE,quote=FALSE,sep=",", row.names=TRUE, col.names=FALSE)

### Append real individuals ###
fname <- dir(pattern='^RealIndividuals')[1]
write.table(out$zout,file=fname,append=TRUE,quote=FALSE,sep=",", row.names=TRUE, col.names=FALSE)

### Append sex of all individuals ###
fname <- dir(pattern='^SexIndividuals')[1]
write.table(out$Sexout,file=fname,append=TRUE,quote=FALSE,sep=",", row.names=TRUE, col.names=FALSE)

### Append gof data ####
fname <- dir(pattern='^gofdata')[1]
write.table(out$gof.data,file=fname,append=TRUE,quote=FALSE,sep=",", row.names=TRUE, col.names=FALSE)

### Append gof new ###
fname <- dir(pattern='^gofnew')[1]
write.table(out$gof.new,file=fname,append=TRUE,quote=FALSE,sep=",", row.names=TRUE, col.names=FALSE)

### Append Log Likelihood ###
fname <- dir(pattern='^LogLike')[1]
write.table(out$likelihood,file=fname,append=TRUE,quote=FALSE,sep=",", row.names=TRUE, col.names=FALSE) 

### Info file - a new one is created ### 

print("Error trap 3.5")
		
#fname <- paste(folderName,"/Info_",ts,"CH",ch,".txt",sep="")
#sink(file=fname)
#cat("Analysis Duration:",duration)
#sink()
print("Error trap 3.6")
	}else{
### Extract output into files ###
ts <- format(Sys.time(), "%y%m%d_%H%M%S")
## Create a folder ##
folderName <- paste("SHOresultsModel_",modelno,"_", ts,"CH",ch,sep="")
dir.create(path=folderName)
## Store MCMC history ##
fname <- paste(folderName,"/SHOmcmchist_",ts,"CH",ch,".csv",sep="")
